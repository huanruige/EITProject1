...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=8982, k_size=[3, 3, 3, 3]):
        self.inplanes = 64
...
# 8982是Eidors生成数据建模决定的 电导率格子数量
def eca_resnet18(k_size=[3, 3, 3, 3], num_classes=8982, pretrained=False):
    model = ResNet(ECABasicBlock, [2, 2, 2, 2], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model


def eca_resnet34(k_size=[3, 3, 3, 3], num_classes=8982, pretrained=False):
    model = ResNet(ECABasicBlock, [3, 4, 6, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model


def eca_resnet50(k_size=[3, 3, 3, 3], num_classes=8982, pretrained=False):
...
def eca_resnet101(k_size=[3, 3, 3, 3], num_classes=8982, pretrained=False):
    model = ResNet(ECABottleneck, [3, 4, 23, 3], num_classes=num_classes, k_size=k_size)
    model.avgpool = nn.AdaptiveAvgPool2d(1)
    return model


def eca_resnet152(k_size=[3, 3, 3, 3], num_classes=8982, pretrained=False):
